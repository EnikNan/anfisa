...
def what_conclusion(parsed_temperature):
    try:
        ...
        if temperature < 18:
            return 'Хей, слишком холодно. Побереги себя, я волнуюсь.'
        elif 18 <= temperature <= 27:
            return 'А с такой температурой мороженное в самый раз! Приятного!'
        else:
            return 'Ух как жарко, скорее по мороженному!'
            
    except ValueError:
        return 'К сожалению, не могу узнать погоду( Что-то с сервисом. Так что выбор за тобой)'
